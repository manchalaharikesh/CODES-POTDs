...
//
// Approach: 1. The brute force solution is a backtracking algorithm. Thus to prevent exploring positions reptitively we store the solution at that point in dp matrix.
//				At dp[i][j] store the minimum sum possible with falling path with non-zero shifts from (i, j)th point. Apply a backtracking algorithm and continue if and only if dp[i][j] doesn't have a solution marked.
